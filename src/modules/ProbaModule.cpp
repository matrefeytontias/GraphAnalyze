...
void setupField() {
    ...
    ImVec4 color;
    if(somme == 1.0f)
        color = ImVec4(.0f,1.0f,.0f,1.0f);
    else
        color = ImVec4(1.0f,.0f,.0f,1.0f);
    ...
    for(int i=0; i<possibility; i++) {
        ImGui::InputFloat(("Probability of possibility " + std::to_string(i+1)).c_str(),&proba[i]);
    }
    ...
    if(somme == 1.0f) {
      ...
      if(selectedPossibility > 0 && selectedPossibility <= possibility){
        ...
        for(int i =0;i<=repetition;i++){
          ...
          std::cout<<histogramData.size()<<std::endl;
        }
...
void addNode(int level,int number, ImVec2 treeSize,ImVec2 fullTreeSize,int numberNode,int count ) {
    ...
    for(unsigned int i =0 ; i<strlen(name);i++){
      if(name[i]!=' '){
      std::cout<<&name[i]<<"  "<<proba[atoi(&name[i])%10-1]<<std::endl;
